<template>
    <section id="payrollSupervisedGroupsFormComponent">
        <a class="btn-simplex btn-simplex-md btn-simplex-primary" href="" title="Registros de grupos de supervisados"
            data-toggle="tooltip"
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <div class="form-group is-required">
                                    ...
                                    <input type="text" id="code" placeholder="Código" class="form-control input-sm"
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="form-group is-required">
                                    ...
                                    <select2 :options="payroll_staffs" v-model="record.supervisor_id">
                                    </select2>
                                </div>
                                <!-- ./Supervisor -->
                            </div>
                            <div class="col-md-6">
                                ...
                                <div class="form-group is-required">
                                    ...
                                    <select2 :options="payroll_staffs" v-model="record.approver_id">
                                    </select2>
                                </div>
                                <!-- ./Aprobador -->
                            </div>
                            <div class="col-md-6">
                                <div class="form-group is-required">
                                    ...
                                    <v-multiselect data-toggle="tooltip"
                                        ...
                                        track_by="text" :hide_selected="false" :group_values="'group'"
                                        :group_label="'label'" :group_select="group_select"
                                        :close_on_select="group_select" :options="payroll_staffs_grouped" :limit="3"
                                        v-model="record.supervised">
                                    </v-multiselect>
                                </div>
                                <div class="float-right">
                                    <button class="btn btn-sm btn-primary btn-custom" data-toggle="tooltip"
                                        type="button" title="Importar" @click="setFile()">
                                        <i class="fa fa-upload"></i>
                                    </button>
                                    <button class="btn btn-sm btn-primary btn-custom" data-toggle="tooltip"
                                        type="button" title="Exportar" @click="exportSupervisedGroups()">
                                        <i class="fa fa-download"></i>
                                    </button>
                                    <input type="file" id="import_data" ref="fileInput" class="nodisplay"
                                        @change="importSupervisedGroups()" />
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="modal-body modal-table">
                        <v-client-table :columns="columns" :data="records" :options="table_options">
                            <div slot="supervisor" slot-scope="props">
                                <span>{{ props.row.supervisor.first_name + ' ' + props.row.supervisor.last_name
                                    }}</span>
                            </div>
                            ...
                            <div slot="id" slot-scope="props" class="text-center">
                                <button title="Lista de Supervisados" data-toggle="modal" data-placement="bottom"
                                    type="button" class="btn btn-info btn-xs btn-icon btn-action btn-tooltip"
                                    @click="initModal(props.row, $event)" data-original-title="Ver registro"
                                    data-target="#supervisorModalList"><i class="fa fa-eye"></i></button>
        ...
        <!--modal lista de supervisados  -->
        <div>
            <!-- Modal -->
            <div class="modal fade" id="supervisorModalList" tabindex="-1" role="dialog"
                aria-labelledby="exampleModalLongTitle">
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLongTitle">Lista de Supervisados</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        ...
                        <div class="modal-body">
                            <ul>
                                <li v-for="group in model_supervised" :key="group.id">
                                    {{ group.text }}
                                </li>
                            </ul>
                        </div>
    ...
    methods: {
         ...
         */
         importSupervisedGroups() {
            ...
            axios
                .post(`${window.app_url}/payroll/supervised-group/import`, formData, {
                    headers: {
                        "Content-Type": "multipart/form-data",
                    },
                })
                .then((response) => {
                    vm.record.supervised = [];

                    response.data.forEach((item) => {
                        vm.payroll_staffs_grouped.forEach((element) => {
                            element.group.forEach((staff) => {
                                if (staff['text'].includes(item.cedulas)) {
                                    vm.record.supervised.push(staff);
                                }
                            })
                        })
                    })
        ...
        exportSupervisedGroups() {
            const vm = this;

            axios
                .post(
                    `${window.app_url}/payroll/supervised-group/export`,
                    { params: vm.record.supervised },
                    { responseType: "blob" }
                )
                .then((response) => {
                    ...
                    link.href = url;
                    link.setAttribute("download", "supervised-groups.xlsx");
        ...
        initModal(row, event) {
            let vm = this;
            vm.model_supervised = [];
            for (const value of row.payroll_supervised_group_staff) {
                vm.model_supervised.push({
         ...
         * y la nueva a registrar(esta funcion fue modificada del base para que la pantallade carga 
         * solo se termine una vez terminado la llamada haciendo mas natural la vista)
         *
         * @author  Ing. Roldan Vargas <rvargas@cenditel.gob.ve> | <roldandvg@gmail.com>
         *
         ...
         */
         async addRecord(modal_id, url, event) {
            ...
            vm.loading = true;
            await vm.initRecords(url, modal_id);
        },
        /**
         * Inicializa los registros base del formulario(esta funcion fue modificada del base para que la pantallade carga 
         * solo se termine una vez terminado la llamada haciendo mas natural la vista)
         *
         * @author Ing. Roldan Vargas <rvargas@cenditel.gob.ve> | <roldandvg@gmail.com>
         *

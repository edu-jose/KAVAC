...
/**
 * @class PayrollCreatePayrollProcessFormulaFunction
 ...
 */
class PayrollCreatePayrollProcessFormulaFunction extends Migration
{
    /**
     * Run the migrations.
     *
            ...
            BEGIN
                ...
                IF POSITION('VACATION_DAYS' IN formula_text) > 0 THEN
                    ...
                    BEGIN
                        ...
                        SELECT vacation_days, assign_to
                        INTO vacation_days_value
                        FROM payroll_vacation_policies
                        WHERE id = (
                            SELECT id
                            FROM payroll_staff_assign_to_function(
                                concept_id,
                                jsonb_build_array(assign_to),
                                '{}'::JSONB,
                                period_start,
                                period_end,
                                '{}'
                ...
                IF POSITION('ADDITIONAL_DAYS_PER_YEAR' IN formula_text) > 0 THEN
                    ...
                    BEGIN
                        -- Obtener el valor de ADDITIONAL_DAYS_PER_YEAR desde payroll_vacation_policies
                        SELECT additional_days_per_year, assign_to
                        INTO additional_days_per_year_value
                        FROM payroll_vacation_policies
                        WHERE id = (
                            SELECT id
                            FROM payroll_staff_assign_to_function(
    ...
    {
        DB::unprepared('DROP FUNCTION IF EXISTS process_formula;');
        DB::unprepared('DROP FUNCTION IF EXISTS get_parameter_value(TEXT, INT, DATE, DATE, INT, INT, JSONB);');
        DB::unprepared('DROP FUNCTION IF EXISTS process_parameter_formula(TEXT, INT, DATE, DATE, INT, INT, JSONB);');
    }
}

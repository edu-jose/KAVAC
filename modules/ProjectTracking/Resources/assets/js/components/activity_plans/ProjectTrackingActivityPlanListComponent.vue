    ...
    <div>
        <v-client-table :columns="columns" :data="records" :options="table_options" ref="tableResults">
            <div slot="name" slot-scope="props" class="text-center">
                <div v-if="props.row.subproject_name && props.row.project_name && props.row?.sub_project">
                    {{ props.row?.sub_project ? props.row.sub_project.name : '' }}
                </div>
                <div v-else-if="props.row.project_name && props.row?.project">
            ...
            <div slot="responsable_name" slot-scope="props" class="text-center">
                <div v-if="props.row.project_name && props.row.project">
                    {{ props.row.project.responsable.first_name ? props.row.project.responsable.first_name :
                        props.row.project.responsable.name }}
                    {{ props.row.project.responsable.last_name }}
                </div>
                <div v-else-if="props.row.product_name && props.row.product">
                    {{ props.row.product.responsable.first_name ? props.row.product.responsable.first_name :
                        props.row.product.responsable.name }}
                    {{ props.row.product.responsable.last_name }}
                </div>
                <div v-else>
                    {{ props.row.sub_project.responsable.first_name ? props.row.sub_project.responsable.first_name :
                        props.row.sub_project.responsable.name }}
        ...
        <project-tracking-activity-plan-info ref="ActivityPlanInfo">
...
export default {
    ...
    methods: {
        ...
        setDetails(ref, id, modal, var_list = null) {
            ...
            vm.$refs[ref].id = id;
            vm.$refs[ref].payroll_staffs = vm.payroll_staffs;

            let activity_number = vm.$refs[ref].record.activities.length;

            for (let team of vm.$refs[ref].record.teams) {
                let employer_name;
                let employer_id;

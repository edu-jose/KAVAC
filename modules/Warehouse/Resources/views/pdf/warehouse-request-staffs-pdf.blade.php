...
<h4 style="font-size: 10rem;">Solicitante: {{ $record?->payrollStaff?->first_name . ' ' . $record?->payrollStaff?->last_name }}</h4>
<h4 style="font-size: 10rem;">Departamento: {{ $record?->department?->name }}</h4>
@foreach ($record?->payrollStaff?->payrollEmployment?->payrollPositions as $position)
    <h4 style="font-size: 10rem;">Cargo: {{ $position->name }}</h4>
@endforeach

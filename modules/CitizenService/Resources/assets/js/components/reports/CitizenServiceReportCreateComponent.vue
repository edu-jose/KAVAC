<template>
    <section id="CitizenServiceReportForm">
        <div class="card-body">
            ...
            <div class="row">
                <div class="col-md-6" id="helpCitizenServiceReportRequestType">
                    <div class="form-group">
                        <label for="citizenserviceRequestTypes">
                            Tipo de trámite
                        </label>
                        <v-multiselect id="citizenserviceRequestTypes" :options="citizen_service_request_types"
                            track_by="text" v-model="record.citizen_service_request_types"
                            :hide_selected="false"></v-multiselect>
                    </div>
                </div>
                <div class="col-md-6" id="helpCitizenServiceReportProcedure">
                    <div class="form-group">
                        <label for="citizenserviceProcedure">
                            Trámite
                        </label>
                        <v-multiselect id="citizenserviceRequest" :options="procedures" track_by="text"
                            v-model="record.procedures" :hide_selected="false"></v-multiselect>
                    </div>
                </div>
                <div class="col-md-6" id="helpCitizenServiceReportStates">
                    <div class="form-group">
                        ...
                        <v-multiselect id="requestState" :options="citizen_service_states" track_by="text"
                            v-model="record.citizen_service_states" :hide_selected="false"></v-multiselect>
                    </div>
                </div>
                <div class="col-md-6" id="helpCitizenServiceReportDepartment">
                    <div class="form-group">
                        <label for="requestDepartment">Departamento:</label>
                        <v-multiselect id="requestDepartment" :options="departments" track_by="text"
                            v-model="record.departments" :hide_selected="false"></v-multiselect>
                    </div>
                </div>
            </div>
            <div class="row">
            ...
            <div class="row">
                <div class="col-md-12">
                    <button type="button" @click="filterRecords()" class="btn btn-sm btn-info float-right"
            ...
            <v-client-table :columns="columns" :data="records" :options="table_options">
                ...
                <div slot="citizen_service_request_type_id" slot-scope="props" class="text-center">
                    <span>{{
                        props.row.citizen_service_request_type
                            ? props.row.citizen_service_request_type.name
                            : ""
                    }}</span>
                </div>
            </v-client-table>
        ...
        <div class="card-footer text-right">
            <div class="row">
                <div class="col-md-3 offset-md-9" id="helpParamButtons">
                    <button type="button" class="btn btn-sm btn-primary btn-custom" @click="createReport()">
...
export default {
    ...
    methods: {
        ...
        async getCitizenServiceDepartments() {
            this.departments = [];
            await axios
                .get(`${window.app_url}/citizenservice/get-departments`)
                .then((response) => {
        ...
        filterRecords() {
            const vm = this;
            let url = `${window.app_url}/citizenservice/reports/search`;
            url +=
                "?type_search=" +
                vm.record.type_search +
                "&start_date=" +
                vm.record.start_date +
                "&end_date=" +
                vm.record.end_date +
                "&citizen_service_request_types=" +
                JSON.stringify(vm.record.citizen_service_request_types) +
                "&citizen_service_states=" +
                JSON.stringify(vm.record.citizen_service_states) +
                "&date=" +
                vm.record.date +
                "&departments=" +
                JSON.stringify(vm.record.departments) +
                "&procedures=" +
                JSON.stringify(vm.record.procedures);
